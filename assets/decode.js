...
function decode(text){
  ...
  output.timeSesReceived = timeSesReceivedS == 0 ? null : dayjs.unix(timeSesReceivedS);
  var timeInreachSentS = extractNum(data, 4);
  output.timeInreachSent = timeInreachSentS == 0 ? null : dayjs.unix(timeInreachSentS);
  return output;
}
...
function render(output) {
  ...
  if (output.weatherItems.length == 0) {
    ...
    for (let i = 0; i < output.weatherItems.length; i++) {
      ...
      td.appendChild(document.createTextNode(output.baseTime.add(i, 'day').format('D MMM')));
      ...
      // Max / Min
      var td = tr.insertCell();
      td.appendChild(document.createTextNode(`${output.weatherItems[i].max}° / ${output.weatherItems[i].min}°`));

      // Precip
      var td = tr.insertCell();
      td.appendChild(document.createTextNode(`${output.weatherItems[i].rainfall} mm`));

      // Wind
      var td = tr.insertCell();
      td.appendChild(document.createTextNode(`${output.weatherItems[i].wind} km/h`));
    }
  }
  ...
  if (output.gaugeItems.length == 0) {
    ...
    for (let i = 0; i < output.gaugeItems.length; i++) {
      ...
      // Date
      var readingTime = output.baseTime.subtract(output.gaugeItems[i].readingTimeDiffMins, 'minute');
      var td = tr.insertCell();
      td.classList.add("mdl-data-table__cell--non-numeric");
      var dateText = document.createElement('div');
      dateText.appendChild(document.createTextNode(readingTime.format('D MMM h:m A')));
      ...
      // Reading
      var td = tr.insertCell();
      td.appendChild(document.createTextNode(readingWithUnits(output.gaugeItems[i].reading, output.gaugeId)));
    }
  }

  // Fill text
  document.getElementById("gauge-name").textContent = `${gaugeData[output.gaugeId].data_source} - ${gaugeData[output.gaugeId].name}`;
  document.getElementById("gauge-id-debug").textContent = output.gaugeId;
  document.getElementById("time-ses-received").textContent = output.timeSesReceived.fromNow();
  document.getElementById("time-ses-received-abs").textContent = output.timeSesReceived.toString();
  document.getElementById("base-time").textContent = output.baseTime.fromNow();
  document.getElementById("base-time-abs").textContent = output.baseTime.toString();
  document.getElementById("time-inreach-sent").textContent = output.timeInreachSent.fromNow();
  document.getElementById("time-inreach-sent-abs").textContent = output.timeInreachSent.toString();
}
...
// var example = "uulBZRYlAgQBACwEBARFAAQIADYEAAQ0AAQVABYE4wMrACUKABME4AMdACUpXI8/VQFmZoY/XwCy4EBlu+lBZQ==";
